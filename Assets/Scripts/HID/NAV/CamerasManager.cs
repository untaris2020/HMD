...
public class CamerasManager : MonoBehaviour
{
    ...
    public GameObject RearviewImageHolder;
    ...
    private const string MY_IP = "192.168.1.114";

    public void SpawnRearviewCam()
    {
        RearviewImageHolder.SetActive(true);

    }

    public void DestroyRearviewCam()
    {
        RearviewImageHolder.SetActive(false);
    }
    ...
    void Update()
    {
        ...
        thisTexture = new Texture2D(320, 240);
        thisTexture.LoadImage(bytesConv);
        RearviewImageHolder.GetComponent<RawImage>().texture = thisTexture;
    ...
    private void ListenForIncommingRequests()
    {
        try
        {
            ...
            tcpListener = new TcpListener(IPAddress.Parse(MY_IP), 5060);
            tcpListener.Start();
                ...
                using(connectedTcpClient = tcpListener.AcceptTcpClient())
                {
                    ...
                    using (NetworkStream stream = connectedTcpClient.GetStream())
                    {
                        ...
                        while ((length = stream.Read(bytes, 0, bytes.Length)) != 0)
                        {
                            ...
                            if(length == clientMessage.Length)
                            {
                                transfer = clientMessage;
                                prevFrame = clientMessage;
                            }
                            else
                            {
                                transfer = prevFrame;
                                //Debug.Log("LOW FRAME: " + incommingData.Length);
                            }
