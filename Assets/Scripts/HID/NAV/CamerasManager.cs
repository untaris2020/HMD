...
public class CamerasManager : MonoBehaviour
{
    ...
    public void Update()
    {
        if (DebugManager.Instance.GetSimulatorMode() && ImageHolder.activeSelf) {

            RenderTexture tmp = null;
            if (navManager.getHeadCam()) {
                tmp = rearViewCamRenderTexture;
            } else if (navManager.getGloveCam()) {
                tmp = gloveCamRenderTexture;
            }

            thisTexture = toTexture2D(tmp);

            ImageHolder.GetComponent<RawImage>().texture = thisTexture;
        }

        else if(newFrame && ImageHolder.activeSelf)
        {
            newFrame = false; 
            try
            {
